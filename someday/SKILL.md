...
- **마스터 파일**: `~/todo/someday.md` (`## 모아둠` 단일 섹션 — 2026-05-01 청소 후 승격됨/드롭 archive 폐기)
...
- "**승격**" / "todo 로 올려" / "진짜 할거됐어" → 모아둠에서 _제거_ + todos.md `## 진행중` 섹션 맨 위에 추가 (someday 에 흔적 X)
- "**드롭**" / "취소" / "안 할 거야" → 모아둠에서 _제거_ + todos.md `## 보류/취소` 섹션 끝에 `- [-] ~~제목~~ (취소: YYYY-MM-DD, someday→drop) — 사유` 추가
- "**수정**" / "내용 바꿔" → 항목 본문 갱신
...
- `[ ]` 빈 체크박스로 추가
- **승격 시** (✨ 2026-05-01 정책 변경) → 모아둠에서 _제거_ + `~/todo/todos.md` `## 진행중` 섹션 맨 위에 추가. someday.md 에는 흔적 남기지 않음 (history 는 git log). 이전엔 `## 승격됨` archive 섹션에 옮겼으나 someday.html noise 라 폐기.
- **드롭 시** (✨ 2026-05-01 정책 변경) → 모아둠에서 _제거_ + `~/todo/todos.md` `## 보류/취소` 섹션 끝에 `- [-] ~~제목~~ (취소: YYYY-MM-DD, someday→drop) — 사유` 형식으로 추가. 같은 cancelled.html 에서 보임. 이전엔 someday.md `## 드롭` archive 섹션에 옮겼으나 폐기.
- **someday.md 는 `## 모아둠` 단일 섹션만 유지** — `## 승격됨` / `## 드롭` 섹션 자체 안 만듦.
