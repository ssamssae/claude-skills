...
3. Mac 본진에 트리거 송신 → Mac 의 `/land` 가 받아서 clone + iOS 빌드 + /irun
4. 전 과정 텔레그램 보고
...
- 대상: `user@100.74.85.37` (Tailscale IP, 맥 본진)
- 명령:
  ```bash
  ssh -o ConnectTimeout=10 -o BatchMode=yes user@100.74.85.37 "cc -p '/land https://github.com/ssamssae/<앱명>.git'"
  ```
- exit 0 이면 성공. 맥 쪽이 받았음을 텔레그램으로 보고.
- exit != 0 이면 다음 Fallback 으로.

### 7. Fallback — 텔레그램 트리거

- Mac SSH 실패 시 (네트워크 / Tailscale 드롭 / ssh key 문제 등):
  - 강대종님 텔레그램으로 단 한 줄만 전송: `/land https://github.com/ssamssae/<앱명>.git`
...
- chat_id: 538806975
- 단계별 상태를 짧게 (시작 / 커밋 / push / Mac 트리거 or Fallback / 완료)
