...
description: 마크다운 파일을 네이버 블로그(SmartEditor ONE)에 자동 발행. Mac 본진 Playwright MCP 내장 Chromium + 강대종 본인 자동로그인 세션 활용. 기본 비공개(closed). 트리거 "/naver-publish <md_path>", "네이버에 올려", "/naver-publish".
---

# 네이버 블로그 자동 발행 v1

마크다운 1개를 강대종 본인 네이버 블로그(`ssamssae`)에 자동 발행한다. 본진 Mac 한정. 사람 빈도(1일 1~2건) + 본인 계정 + 기본 비공개. 4조건 중 하나라도 깨지는 요청이 오면 NO GO 회귀.
...
3. 입력 인자 파싱:
   ...
   - `--category <name>` (옵션)
   - `--open all|closed|neighbor` (옵션, **기본 `closed`**)
...
발행 모달이 뜨면 카테고리/태그/공개설정 옵션 노출. **기본은 `비공개`** 라디오를 선택.

발견 시 정확한 셀렉터/라디오 라벨 기록 후 evaluate 로 클릭. 모달 selector 패턴은 직접 inspect 후 SKILL.md 에 적어둔다 (v1 검증 후 업데이트). 잠정 (확정 필요):
- 비공개 라디오 라벨 텍스트 = `비공개`
- evaluate 로 `Array.from(doc.querySelectorAll('label, input[type=radio]')).find(l => /비공개/.test(l.innerText || l.value))` 식으로 찾기
...
**중요한 단서:** `--open closed` (비공개) 일 때는 공개 URL fetch 가 안 된다 (인증 필요). 그래서 검증을 Playwright 의 **이미 로그인된** 세션 안에서 한다:

1. Playwright 의 같은 탭/새 탭에서 `https://blog.naver.com/ssamssae/<postNo>` 진입
2. evaluate 로 article DOM 의 제목/본문 첫 30자 매치 검사
3. 제목 정확 일치 + 본문 첫 30자 일치 → **PASS**

`--open all` (전체공개) 인 경우는 인증 없이 fetch 도 가능 (보너스 검증 추가 가능).
...
- 4조건 (본인계정/본인세션/사람빈도/closed기본) 중 1개라도 깨지는 요청이 오면 즉시 NO GO 회귀.
- 캡차/2FA 발견 시 우회 시도 X. 강대종 결정 대기.
- 검증용 테스트 글은 closed (비공개)로. 풀그린 후 강대종이 공개/비공개/삭제 결정.
