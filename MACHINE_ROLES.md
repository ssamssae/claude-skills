...
| 🧪 **노트북 3060** | `desktop-4mnj1c0` | 온디맨드 실습실/샌드박스 (위험 자동화 실험, GPU 보조) · 클라이언트 only · 인바운드 0 | 필요 시만 | — |
...
- **3060 두 대 모두 클라이언트 only — sshd disable, Tailscale 합류 (2026-05-02), 인바운드 0, outbound only.** 다른 노드에서 진입 차단, 본인이 outbound 로 ssh m1/mb/w 만 사용.
